...
from .serializers import ProductSerializer, StockItemSerializer, InventoryLogSerializer
import csv
import io
...
def export_inventory(request):
    ...
    try:
        ...
        if format_type == 'csv':
            ...
            response['Content-Disposition'] = f'attachment; filename="inventory_export_{timezone.now().date()}.csv"'
            return response
        ...
        with transaction.atomic():
            for update in updates:
                try:
                    ...
                    cost_price = float(update.get('cost_price', 0)) if update.get('cost_price') else None
                    sale_price = float(update.get('sale_price', 0)) if update.get('sale_price') else None
                    
                    stock_item = get_object_or_404(
                        StockItem.objects.get(id=item_id, merchant=merchant_profile)
                    )
                    ...
                    InventoryLog.objects.create(
                        ...
                        source='BULK_UPDATE',
                        device_id='mobile_app',
                        reason=f'Bulk update: quantity {old_quantity}->{quantity}'
                    )
