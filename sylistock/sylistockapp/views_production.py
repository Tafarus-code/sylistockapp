    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if serializer.is_valid():
                ...
                barcode = serializer.validated_data['product']['barcode']
                product, created = Product.objects.get_or_create(
                    barcode=barcode,
                    defaults={
                        'name': serializer.validated_data['product']['name'],
                        'description': serializer.validated_data['product'].get('description', '')
                    }
                )
...
def inventory_detail_update_delete(request, pk):
    ...
    try:
        ...
        stock_item = get_object_or_404(
            StockItem.objects.get(pk=pk, merchant=merchant_profile)
        )
    ...
    if request.method == 'GET':
        ...
        if serializer.is_valid():
            ...
            if old_quantity != updated_item.quantity:
                action = 'ADJ' if updated_item.quantity < old_quantity else 'IN'
            ...
            return Response(StockItemSerializer(updated_item).data)
        else:
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        ...
        if serializer.is_valid():
            ...
            with transaction.atomic():
                ...
                InventoryLog.objects.create(
                    ...
                    product=product,
                    action=action,
                    quantity_changed=1 if action == 'IN' else -1,
                    source=source,
                    device_id=device_id,
